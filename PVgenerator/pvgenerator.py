...
def generate_pv_series(temperature:pd.DataFrame, irradiance:pd.DataFrame, 
    parameters:dict={
        'Pmpp': 8.18 * 31.2, # maximum power point  = maximum power point current [A] * maximum power point voltage [V]
        'FF': (8.18 * 31.2)/(8.89 * 37.8), # fill factor = maximum power point [W] / (short circuit current [A] * open source voltage [V])
        'Voc': 37.8, # open circuit current [V]
        'Isc': 8.89, # short circuit current [A]
        'T0': 25+273.15, # standard module temperature [K]
        'E0': 1000, # standard irradiance [W/m^2]
        'eta_inv': 0.9, # inverter efficiency [-]
        'NOCT': 45.7, # nominal operating cell temperature [degreeC]
        'module_area': 1.6, # size of module
        'module_number':1 # amount of modules
    }, measurementfilter:dict={
        'maximum_temperature':40, # maximum temperature [degreeC]
        'maximum_irradiance':1000 # maximum irridation [W/m^2]
    }):
    """
...
if __name__ == "__main__":
    ...
    photovoltaic_generation = generate_pv_series(temperature, irridation)
    
    # store data:
    photovoltaic_generation.to_excel('PV generation.xlsx')
